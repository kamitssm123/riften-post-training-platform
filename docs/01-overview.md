...
generated locally by `backend/generate_corpus.py`; nothing here talks to a
real model provider or real user traffic.

## System diagram

```
                    ┌─────────────────────────┐
                    │  generate_corpus.py     │   random.seed(42)
                    │  (backend/)              │
                    └────────────┬─────────────┘
                                 │ writes
                                 ▼
                    ...
                    /data/traces.db  (SQLite, single `traces` table)
                                 ...
                                 ▼
                    ┌─────────────────────────┐
                    │  main.py (FastAPI)       │  uvicorn main:app --port 8000
                    │  GET  /traces            │
                    │  GET  /traces/{id}       │
                    │  GET  /sessions/{id}     │
                    │  GET  /stats             │
...
The export scripts (`export_sft.py`, `export_preference.py`,
`exclusion_report.py`) read the same SQLite database as the API and can be
run either as standalone scripts or triggered through the API's `POST`/`GET`
`/export/*` routes — both paths call the same Python functions
...
| Database | SQLite (stdlib `sqlite3`) | bundled with Python 3.9.6 | The corpus is 381 rows in one table (see `docs/06-dataset.md`); there are no concurrent writers (ingest is a single wipe-and-reload batch job) and no need for a server process — a file at `/data/traces.db` is the entire deployment story. `sqlite3`'s JSON1 functions (`json_each`, `json_extract`) are used directly in `main.py`'s `errored` filter to reach into the `tool_calls` JSON column without a second table. |
| Test runner | pytest | 8.4.2 | 31 tests across 4 files use `monkeypatch` to redirect each module's file-path globals (`DB_PATH`, `OUT_PATH`, `EXPORTS_DIR`) into `tmp_path`, so tests never touch the real `/data` directory. |
| HTTP test client | httpx (via `fastapi.testclient.TestClient`) | 0.28.1 | Required by `TestClient` in this FastAPI version; used only in `test_main.py`. |
| Frontend framework | React | 19.2.8 | Function components + hooks (`useState`/`useEffect`) are the entire state model in `App.tsx` — no router, no global store, because the whole UI is one screen with a right-hand panel that swaps content. |
...
```
/backend    FastAPI app, schema, generator, ingest, exports, tests (Python)
