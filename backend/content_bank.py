    ...
    Topic(
        ...
        exchanges=[
            ...
            Exchange(
                ...
                answers=[
                    "For a missing reset email, check the admin API's `GET /admin/users/{id}/email-log` for delivery status -- most misses are the email landing in spam or an outdated address on the account.",
                    "For a missing reset email, first confirm the address on file is current, then check the delivery log; if it shows as sent, it's almost always a spam-filter issue on the user's side.",
                ],
            ),
        ],
    ),
        ...
        exchanges=[
            Exchange(
                questions=[
                    ...
                    "My webhook keeps failing with a timeout error -- any idea why?",
                ],
                answers=[
                    "Webhook delivery times out when your endpoint takes longer than the 5-second delivery window -- move slow processing to a background job and return a 200 immediately on receipt.",
                    ...
                    "Webhook timeouts almost always mean the receiving endpoint isn't responding within the 5-second window -- offload the actual processing and return 200 right away.",
                    "Webhook delivery gives your endpoint 5 seconds to respond; anything slower gets marked a timeout, so decouple the response from the processing work.",
                ],
            ),
            Exchange(
                questions=[
    ...
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "Changelog summary for release notes: added rate-limit headers to all API responses, fixed a race condition in session refresh, and deprecated the v1 `/search` endpoint in favor of v2.",
                    "Changelog summary for release notes: rate-limit headers now ship on every API response, a session-refresh race condition is fixed, and the v1 `/search` endpoint is deprecated in favor of v2.",
                    "Changelog summary for release notes: this entry covers three things worth calling out -- new rate-limit headers on all responses, a session-refresh race-condition fix, and v1 `/search` deprecation in favor of v2.",
                    "Changelog summary for release notes: rate-limit headers added API-wide, a session-refresh race condition resolved, and the older v1 `/search` endpoint deprecated in favor of v2.",
                ],
            ),
        ],
    ),
        ...
        exchanges=[
            Exchange(
                questions=[
                    ...
                    "How does cursor-based pagination work for a large export?",
                    "My export is too large for one page -- how do I paginate it?",
                ],
                answers=[
                    "For pagination through a large export, use the `cursor` field returned in each page's response and pass it as `?cursor=...` on the next request until the field is null.",
                    "For pagination through a large export, each page returns a `cursor` value -- pass that back as the `cursor` query parameter to fetch the next page, and stop once it comes back null.",
                    "For pagination through a large export, follow the `cursor` field: read it from the response and include it on your next request; a null `cursor` means you've reached the last page.",
                    "For pagination through a large export, follow the `cursor` field in each page of the response, passing it as `?cursor=...` on subsequent calls, until the API returns a null cursor.",
                ],
            ),
            Exchange(
                questions=[
                ...
                answers=[
                    "For going back a page, the cursor is forward-only; you'd need to have saved the previous page's cursor value yourself to return to it.",
                    "For going back a page, there's no reverse cursor -- paginating backwards means re-requesting from the start using a previously saved cursor.",
                ],
            ),
        ],
    ),
    ...
    Topic(
        ...
        exchanges=[
            ...
            Exchange(
                ...
                answers=[
                    "For exponential backoff retry limits, a common cap is 5-6 attempts -- beyond that, the delays get long enough that you should surface the failure instead of retrying further.",
                    "For exponential backoff retry limits, most implementations cap it at around 5 retries; after that the request should fail loudly rather than keep waiting.",
                ],
            ),
        ],
    ),
        ...
        exchanges=[
            Exchange(
                questions=[
                    ...
                    "What status code should I use when throttling a rate-limited request?",
                    "How should a rate-limited response be coded -- what status code applies?",
                ],
                answers=[
                    "For a rate-limited request, the status code is `429 Too Many Requests`, ideally with a `Retry-After` header indicating when the client can try again.",
                    "For a rate-limited request, the status code is `429 Too Many Requests`, paired with a `Retry-After` header so the client knows how long to wait.",
                    "For a rate-limited request, the status code is `429 Too Many Requests` -- include a `Retry-After` header telling the client exactly when it's safe to retry.",
                    "For a rate-limited request, the status code is `429 Too Many Requests` with a `Retry-After` header, rather than a generic 4xx or 5xx.",
                ],
            ),
        ],
    ),
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "CSV to JSON: given the header row and values, here's the resulting JSON object with each column mapped to its corresponding field.",
                    "CSV to JSON: match each CSV header to its value in the row and you get a JSON object where every column becomes a key-value field.",
                    "CSV to JSON: pair each header-row column name with the value in that same position, producing one JSON object per row.",
                    "CSV to JSON: each column from the header becomes a key, and each corresponding cell value becomes that key's field value.",
                ],
            ),
        ],
    ),
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "For duplicate SQL query rows, the join against the `orders` table is one-to-many without aggregation, so each matching order row duplicates the parent record. Add a `GROUP BY` or use a window function.",
                    "For duplicate SQL query rows, this is a classic one-to-many join issue: joining against `orders` without aggregating multiplies the parent row once per matching order. Add a `GROUP BY` (or dedupe with `ROW_NUMBER()`).",
                    "For duplicate SQL query rows, they come from an unaggregated one-to-many join with `orders` -- each order match repeats the base row. Fix it with `GROUP BY` or a window function to collapse the duplicates.",
                    "For duplicate SQL query rows, the query duplicates because the join fans out one-to-many against `orders` with no aggregation. Adding `GROUP BY` on the primary key resolves it.",
                ],
            ),
            Exchange(
                questions=[
    ...
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "The cron expression `0 9 * * 1-5` runs at 9:00 AM Monday through Friday.",
                    "This cron expression works: `0 9 * * 1-5` fires at 9:00 AM on weekdays only, skipping Saturday and Sunday.",
                    "Use the cron expression `0 9 * * 1-5` -- it schedules the job for 9:00 AM every weekday, Monday through Friday.",
                    "The cron expression `0 9 * * 1-5` is what you want: minute 0, hour 9, every day-of-month, every month, weekdays 1-5 (Mon-Fri).",
                ],
            ),
        ],
    ),
        ...
        exchanges=[
            Exchange(
                questions=[
                    ...
                    "What Cache-Control caching settings should I use to avoid stale data?",
                    "How do I control caching behavior with Cache-Control for an API endpoint?",
                    "What's the right Cache-Control caching header setup for a frequently-changing resource?",
                ],
                answers=[
                    "For Cache-Control caching, use `Cache-Control: no-cache` for data that changes often so clients revalidate on every request, or set a `max-age` that matches how long staleness is acceptable for stable data.",
                    "For Cache-Control caching, set `Cache-Control: max-age=<seconds>` for anything safe to serve stale for a bit, and `no-cache` (forcing revalidation) for endpoints where correctness matters more than speed.",
                    "For Cache-Control caching, use a short `max-age` plus `stale-while-revalidate` for endpoints that tolerate brief staleness, or `no-store` entirely for responses that must never be cached.",
                    "For Cache-Control caching, match the header to how tolerant the data is of staleness: `max-age` for mostly-static responses, `no-cache` to force revalidation, `no-store` when caching would actually be wrong.",
                ],
            ),
            Exchange(
                questions=[
    ...
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "For API versioning, introduce the change under a new version path (e.g. `/v2/...`) and keep `/v1/...` fully functional in parallel, giving existing clients a deprecation window before you sunset the old version.",
                    "For API versioning, version the endpoint (`/v2/...`) rather than mutating `/v1/...` in place, so existing integrations keep working unchanged until you formally deprecate and eventually retire v1.",
                    "For API versioning, the safe pattern is additive: ship the breaking change as `/v2/...`, leave `/v1/...` untouched, and communicate a clear deprecation timeline before removing v1.",
                    "For API versioning, avoid breaking clients by putting the change behind a new version path, running old and new versions side by side, and deprecating the old one only after clients have had time to migrate.",
                ],
            ),
        ],
    ),
    ...
    Topic(
        ...
        exchanges=[
            ...
            Exchange(
                ...
                error_answers=[
                    "prod-db-2: I tried to re-check disk usage but the monitoring call failed, so I can't confirm it's back to normal yet.",
                ],
            ),
        ],
    ),
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "For jane@example.com, I found the customer record and upgraded them to the Pro plan.",
    ...
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                error_answers=[
                    "For the payments deploy check, I hit a lookup error and can't confirm the last deploy's status.",
                    "For the payments deploy check, the deploy-history lookup failed, so I wasn't able to retrieve the recent deploy history.",
                ],
            ),
            Exchange(
                questions=[
                ...
                error_answers=[
                    "For that payments deploy's logs, I hit a lookup error and can't confirm whether there were errors.",
                ],
            ),
        ],
    ),
    Topic(
        ...
        exchanges=[
            Exchange(
                ...
                answers=[
                    "Ingestion worker logs: the top recurring error is a connection timeout to the upstream queue, occurring roughly every 10 minutes.",
                    ...
                    "Ingestion worker logs: the main problem is a recurring connection timeout to the upstream message queue, roughly every 10 minutes.",
                    "Ingestion worker logs: the top error is the ingestion worker timing out connecting to the upstream queue, roughly every 10 minutes.",
                ],
                tool=ToolSpec(
                    name="fetch_logs",
                ...
                error_answers=[
                    "Ingestion worker logs: I tried to fetch them but the log service returned an error, so I couldn't retrieve or summarize anything.",
                    "Ingestion worker logs: the fetch failed -- I don't have visibility into the current errors.",
                ],
            ),
            Exchange(
                questions=[
                ...
                answers=[
                    "Yes -- the ingestion worker's timeout frequency is up about 20% over the last 24 hours.",
                    "It's trending worse: the ingestion worker's timeouts have increased roughly 20% in the past day.",
                ],
                tool=ToolSpec(
                    name="fetch_logs",
                    args={"service": "ingestion-worker"},
                ...
                error_answers=[
                    "For the ingestion worker's timeout trend, I hit a log-service error and can't confirm it right now.",
                ],
            ),
        ],
    ),
